...
// FILE: hw7_test.cpp
// DATE: Spring 2022
// DESC: Basic unit tests for HW 7
//---------------------------------------------------------------------------
...
TEST(BasicHashMapTests, SimpleContainsCheck)
{
  HashMap<char,int> m;
  m.insert('a', 10);
  m.insert('b', 20);
...
TEST(BasicHashMapTests, MoreInvolvedContainsCheck)
{
  HashMap<char,int> m;
  for (int i = 'z'; i >= 'a'; --i) 
    m.insert(i, (int)i);
...
TEST(BasicHashMapTests, NextKeyCheck)
{
  HashMap<char,int> m;
  m.insert('e', 50);
  m.insert('a', 10);
...
TEST(BasicHashMapTests, PrevKeyCheck)
{
  HashMap<char,int> m;
  m.insert('f', 50);
  m.insert('b', 10);
...
TEST(BasicHashMapTests, StatsEmptyCheck)
{
  HashMap<int,int> m;
  ASSERT_EQ(0, m.size());
  ASSERT_EQ(0, m.min_chain_length());
  ASSERT_EQ(0, m.max_chain_length());
  ASSERT_EQ(0.0, m.avg_chain_length());
}

TEST(BasicHashMapTests, StatsBiasedCheck)
{
  int n = 24;
  HashMap<int,int> m;
  for (int i = 0; i < n; ++i)
    m.insert(i % 8, i);
  ASSERT_EQ(n, m.size());
  ASSERT_EQ(3, m.min_chain_length());
  ASSERT_EQ(3, m.max_chain_length());
  ASSERT_EQ(3.0, m.avg_chain_length());
}

TEST(BasicHashMapTests, StatsRandomCheck)
{
  int n = 48;
  std::srand(13);
  HashMap<int,int> m;
  ...
  for (int i = 0; i < n; ++i)
    m.insert(std::rand(), i);
  ASSERT_EQ(n, m.size());
  ASSERT_EQ(1, m.min_chain_length());
  ASSERT_EQ(4, m.max_chain_length());
  ASSERT_EQ(1.5, m.avg_chain_length());
}
